...
@click.command()
@click.option(
    "--config", default="configs/split.yaml", help="Path to split config.yaml"
)
...
@click.option(
    "--test-size", default=None, type=float, help="Override test_size from config"
)
def main(config, root, raw, splits, seed, test_size):
    ...
    if os.path.exists(config):
        ...
        test_size = (
            ...
            else cfg.get("split", {}).get("test_size", 0.2)
        )
    else:
    ...
    if not os.path.exists(raw_path):
        raise FileNotFoundError(
            f"Raw data not found at {raw_path}. Place tweets_train.csv there."
        )
    ...
    click.echo(
        f"Created splits: {len(df_train)} train / {len(df_dev)} dev (seed={seed}, test_size={test_size})"
    )
