...
def fit_eval_svm(
    svm, params, X_train, y_train, X_val, y_val, metric: Literal["f1_score", "rmse"]
):
...
def run_grid_search(
    svm, X_train, y_train, X_val, y_val, param_grid, metric: Literal["f1_score", "rmse"]
):
    results = []
...
def main():
    ...
    if not os.path.exists(train_path) or not os.path.exists(dev_path):
        print(
            f"Missing {train_path} or {dev_path}. Generate BERT features first via predict with FeatureWriter."
        )
        return
