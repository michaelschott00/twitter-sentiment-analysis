...
class _BaseDataset(Dataset):
    ...
    def __init__(
        self, root_dir: str, split: Literal["train", "dev", "test_1", "test_2"]
    ) -> None:
        super().__init__()
        ...
        for column in self.df.columns:
            if (self.df[column].dtype.name == "category") and not (
                column == "sentiment"
            ):
        ...
        if "test" not in self.split:
            self.df.sentiment = self.df.sentiment.astype("category").apply(
                lambda x: self.LABEL_CODING[x]
            )
    ...
    def __getitem__(
        self, idx: int
    ) -> tuple[dict[str, int | str], dict[str, int | float | None]]:
...
class TwitterDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        assert not (
            self.hparams.features == "contrast" and self.hparams.labels != "clf"
        ...
        self.preprocessing = (
            nn.Sequential(*preprocessing) if len(preprocessing) > 0 else None
        )
        ...
        self.augmentation = (
            nn.Sequential(*augmentation) if len(augmentation) > 0 else None
        )
    ...
    def setup(self, stage: str):
        ...
        if stage == "fit":
            self.twitter_train = Trainset_cls(
            ...
            if self.hparams.external_data_paths:
                ...
                self.twitter_train = torch.utils.data.ConcatDataset(
                    [self.twitter_train] + external_datasets
                )

            self.twitter_dev = Devset_cls(
                root_dir=self.hparams.root_dir,
                split="dev",
                preprocessing=self.preprocessing,
            )
        ...
        # Load test data
        if stage == "predict":
            self.twitter_test_1 = PredictionTextDataset(
                root_dir=self.hparams.root_dir,
                split="test_1",
                preprocessing=self.preprocessing,
            )
            self.twitter_test_2 = PredictionTextDataset(
                root_dir=self.hparams.root_dir,
                split="test_2",
                preprocessing=self.preprocessing,
            )
    ...
    def text_collate_fn(self, batch):
        ...
        encodings["labels"] = torch.tensor(
            [(x["score_compound"], x["sentiment"]) for x in labels], dtype=torch.float32
        )
        return encodings
    ...
    def train_dataloader(self):
        ...
        if self.hparams.class_sample_weights is not None:
            ...
            if self.hparams.labels == "clf":
                samples_weight = torch.from_numpy(
                    np.array([self.hparams.class_sample_weights[t] for t in labels])
                )
        ...
        base_cls = (
            getattr(self.twitter_train, "datasets", [self.twitter_train])[0].__class__
            if isinstance(self.twitter_train, torch.utils.data.ConcatDataset)
            else self.twitter_train.__class__
        )
        collate_fn = self.collate_fn_map.get(
            base_cls, self.collate_fn_map.get(self.twitter_train.__class__)
        )
