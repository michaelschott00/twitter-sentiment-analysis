...
def load_data():
    df = pd.read_csv('data/splits/tweets_train.csv', dtype={'author_id': 'category', 'type': 'category', 'possibly_sensitive': 'category', 'sentiment': 'category'})
    df.words = df.words.apply(lambda x: ' '.join([token for token in eval(x) if token != "amp" and token != "gt" and token != "lt"])).to_numpy()
    return df
...
def select_k_kmeans(df: pd.DataFrame, min_k: int = 1, max_k: int = 100, steps: int = 10, feautres: Literal["tfidf"] = "tfidf"):
...
def select_k_gmm(df: pd.DataFrame, min_k: int, max_k: int, steps: int, covariance: str = "spherical", feautres: Literal["tfidf"] = "tfidf"):
    if feautres == "tfidf":
        vectors, vocab = get_tfidf(df)
    else:
        raise ValueError(f"Features {feautres} not supported.")
...
def add_kmeans_clusters(df: pd.DataFrame, n_clusters: int, feautres: Literal["tfidf"] = "tfidf"):
    if feautres == "tfidf":
...
def add_gmm_components(df: pd.DataFrame,
                       n_components: int,
...
def add_principal_components(df: pd.DataFrame,
                             kernel=None,
                             feautres: Literal["tfidf"] = "tfidf",
                             whiten: bool = False,
                             center: bool = False):
...
def add_tsne_components(df: pd.DataFrame,
...
def plot_lower_dimensional(df: pd.DataFrame, x: str, y: str, hue_column: str, add_style: bool = False):
    if add_style:
        sns.scatterplot(data=df, x=x, y=y, hue=hue_column, style='sentiment', palette='tab10')
    else:
...
def most_polarized(df: pd.DataFrame, column: str, n: int = 3):
    cluster_sentiment = df.groupby(column)['score_compound'].aggregate(np.median).reset_index().sort_values(by='score_compound', ascending=True)
    most_negative = cluster_sentiment[:n]
    most_positive = cluster_sentiment[-n:]
    random_neutral = cluster_sentiment[(cluster_sentiment['score_compound'] < 0.05) & (cluster_sentiment['score_compound'] > -0.05)].sample(n)
